...
export default function DailyInspectionPVGV({ onBack, onSave, user }: DailyInspectionPVGVProps) {
  ...
  const [elecLeakageCurrent, setElecLeakageCurrent] = useState<string>(''); // 0:1.500 mA
  ...
  // Real-time validations - ignore if empty
  ...
  const isElecLeakageCurrentInvalid = elecLeakageCurrent !== "" && ((parseFloat(elecLeakageCurrent) || 0) < 0 || (parseFloat(elecLeakageCurrent) || 0) > 1.500);
  ...
  if (modelProfile === 'GV58A') {
    isDimAInvalid = dimA !== "" && (valA < 0 || valA > 1.5);
    isDimBInvalid = dimB !== "" && (valB < 0 || valB > 1.5);
    isDimCInvalid = dimC !== "" && (valC < 0 || valC > 1.5);
    isDimLInvalid = dimL !== "" && (valL < 0 || valL > 1.5);
    isDimMInvalid = dimM !== "" && (valM < 0 || valM > 1.5);
    isDimNInvalid = dimN !== "" && (valN < 0 || valN > 1.5);
    isDiffDEInvalid = dimD !== "" && dimE !== "" && diffDE > 2.0;
  } else {
    isDimAInvalid = dimA !== "" && (valA < -1 || valA > 0);
    isDimBInvalid = dimB !== "" && (valB < -1 || valB > 0);
    isDimCInvalid = dimC !== "" && (valC < -2 || valC > 0);
    isDimLInvalid = dimL !== "" && (valL < 2.5 || valL > 4.5);
    isDimMInvalid = dimM !== "" && (valM < 2.5 || valM > 4.5);
    isDimNInvalid = dimN !== "" && (valN < 2.5 || valN > 4.5);
    isDiffDEInvalid = dimD !== "" && dimE !== "" && diffDE > 1.0;
  }

  const isDimYInvalid = dimY !== "" && (valY < 9.5 || valY > 10.5);
  const isDimZInvalid = dimZ !== "" && (valZ < 9.5 || valZ > 10.5);
  ...
  // Compute validation states dynamically based on the inputs
  const gFreezerVal = parseFloat(gasketContactFreezer);
  const gCabinetVal = parseFloat(gasketContactCabinet);
  const pForceVal = parseFloat(doorPullForce);
  const tA1 = parseFloat(torqueA1);
  const tA2 = parseFloat(torqueA2);
  const tA3 = parseFloat(torqueA3);
  const tB1 = parseFloat(torqueB1);
  const tB2 = parseFloat(torqueB2);
  const tC1 = parseFloat(torqueC1);
  const tC2 = parseFloat(torqueC2);
  const tT1 = parseFloat(torqueT1);
  const tT2 = parseFloat(torqueT2);

  const isGasketContactFreezerInvalid = gasketContactFreezer !== "" && (isNaN(gFreezerVal) || gFreezerVal < 7);
  const isGasketContactCabinetInvalid = gasketContactCabinet !== "" && (isNaN(gCabinetVal) || gCabinetVal < 7);
  const isDoorPullForceInvalid = doorPullForce !== "" && (isNaN(pForceVal) || pForceVal > 5.0);

  const isTorqueA1Invalid = torqueA1 !== "" && (isNaN(tA1) || tA1 < 4.8 || tA1 > 7.0);
  const isTorqueA2Invalid = torqueA2 !== "" && (isNaN(tA2) || tA2 < 4.8 || tA2 > 7.0);
  const isTorqueA3Invalid = torqueA3 !== "" && (isNaN(tA3) || tA3 < 4.8 || tA3 > 7.0);
  const isTorqueB1Invalid = torqueB1 !== "" && (isNaN(tB1) || tB1 < 4.8 || tB1 > 7.0);
  const isTorqueB2Invalid = torqueB2 !== "" && (isNaN(tB2) || tB2 < 4.8 || tB2 > 7.0);
  const isTorqueC1Invalid = torqueC1 !== "" && (isNaN(tC1) || tC1 < 4.8 || tC1 > 7.0);
  const isTorqueC2Invalid = torqueC2 !== "" && (isNaN(tC2) || tC2 < 4.8 || tC2 > 7.0);
  const isTorqueT1Invalid = torqueT1 !== "" && (isNaN(tT1) || tT1 > 1.5);
  const isTorqueT2Invalid = torqueT2 !== "" && (isNaN(tT2) || tT2 > 2.0);
      ...
      if (stored) {
        ...
        if (lineConfig && lineConfig.submission_url) {
          const formattedPayload = {
            ...
            'ضبط شاشة التحكم على R/5 & F/-18': payload.data.controlPanelR5F18 || '',
            ...
            'احكام تجميع ال R Door Pocket Line B': payload.data.rDoorPocketTightB || '',
            'احكام تجميع ال R Door Pocket Line C': payload.data.rDoorPocketTightC || '',
